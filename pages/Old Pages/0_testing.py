            ...
            with st.container(border=True):
                ...
                with tab1:
                    if not monthly_data.empty:
                        ...
                        with col1:
                            st.plotly_chart(create_bar_chart(monthly_data), use_container_width=True, key=f"bar_{location.index()}")
                        with col2:
                            st.plotly_chart(create_pie_chart(iaat_downtime, total_hours), use_container_width=True, key=f"pie_{location.index()}")
